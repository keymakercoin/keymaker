...
static bool NatpmpMapping(natpmp_t* natpmp, const struct in_addr& external_ipv4_addr, uint16_t private_port, bool& external_ip_discovered)
{
  ...
  const int r_send = sendnewpokeymappingrequest(natpmp, NATPMP_PROTOCOL_TCP, private_port, suggested_external_port, 3600 /*seconds*/);
  if (r_send == 12 /* OK */) {
    int r_read;
    natpmpresp_t response;
    do {
    ...
    if (r_read == 0) {
      auto pm = response.pnu.newpokeymapping;
    ...
    LogPrintf("natpmp: sendnewpokeymappingrequest() failed with %d error.\n", r_send);
  }
...
static bool ProcessNatpmp()
{
  ...
  if (NatpmpInit(&natpmp) && NatpmpDiscover(&natpmp, external_ipv4_addr)) {
    ...
    const int r_send = sendnewpokeymappingrequest(&natpmp, NATPMP_PROTOCOL_TCP, private_port, g_mapport_external_port, /* remove a port mapping */ 0);
    g_mapport_external_port = 0;
    if (r_send == 12 /* OK */) {
      LogPrintf("natpmp: Port mapping removed successfully.\n");
    } else {
      LogPrintf("natpmp: sendnewpokeymappingrequest(0) failed with %d error.\n", r_send);
    }
  }
