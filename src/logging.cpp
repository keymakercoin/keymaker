 ...
 * https://github.com/bitcoin/bitcoin/commit/ee3374234c60aba2cc4c5cd5cac1c0aefc2d817c
 */
...
bool BCLog::Logger::StartLogging() {
    ...
    if (m_print_to_file) {
        ...
        // Add newlines to the logfile to distinguish
        // this execution from the last one.
        FileWriteStr("\n\n\n\n\n", m_fileout);
    }
...
std::vector <LogCategory> BCLog::Logger::LogCategoriesList() const {
    // Sort log categories in Alphabetical order.
...
void
BCLog::Logger::LogPrintStr(const std::string &str, const std::string &logging_function, const std::string &source_file,
    ...
    if (m_log_threadnames && m_started_new_line) {
        // 16 chars total, "keym-" is 4 of them and another 1 is a NUL terminator
...
void BCLog::Logger::ShrinkDebugFile() {
    ...
    if (file && log_size > 11 * (RECENT_DEBUG_HISTORY_SIZE / 10)) {
        ...
        if (fseek(file, -((long) vch.size()), SEEK_END)) {
            LogPrintf("Failed to shring debug log file: fseek(...) failed\n");
            fclose(file);
            return;
        }
