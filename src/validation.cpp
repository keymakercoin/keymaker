...
bool CChainState::ConnectBlock(const CBlock &block, CValidationState &state, CBlockIndex *pindex,
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto &tx: block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(KEYMAKER): transaction %s conflicts with transaction lock %s",
    ...
    nTimeKeymakerSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Keymaker specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4),
             nTimeKeymakerSpecific * MICRO, nTimeKeymakerSpecific * MILLI / nBlocksTotal);
