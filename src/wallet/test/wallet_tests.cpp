...
SetLastBlockProcessed (::ChainActive()
...
CWallet::ScanResult result = wallet.ScanForWalletTransactions({} /* start_block */, {} /* stop_block */, reserver,
                                                              false /* update */);
...
CWallet::ScanResult result = wallet.ScanForWalletTransactions(oldTip->GetBlockHash(), {} /* stop_block */, reserver,
                                                              false /* update */);
BOOST_CHECK_EQUAL(result
.status, CWallet::ScanResult::FAILURE);
BOOST_CHECK_EQUAL(result
.last_failed_block, oldTip->

GetBlockHash()

);
BOOST_CHECK_EQUAL(result
.last_scanned_block, newTip->

GetBlockHash()

);
BOOST_CHECK_EQUAL(*result
.last_scanned_height, newTip->nHeight);
BOOST_CHECK_EQUAL(wallet
.

GetBalance()

.m_mine_immature, 4 * COIN);
}
...
// after.
{
std::shared_ptr <CWallet> wallet = std::make_shared<CWallet>(chain.get(), WalletLocation(),
...
m_coinbase_txns.
emplace_back(CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()))
.vtx[0]);
m_coinbase_txns.
emplace_back(CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()))
.vtx[0]);
...
LOCK(wallet
->cs_wallet);
wallet->mapKeyMetadata[coinbaseKey.
...
nCreateTime = BLOCK_TIME;
wallet->
AddKeyPubKey(coinbaseKey, coinbaseKey
.
...
3U);
BOOST_CHECK_EQUAL(m_coinbase_txns
.
...
static int64_t AddTx(CWallet &wallet, uint32_t lockTime, int64_t mockTime, int64_t blockTime) {
    ...
    if (blockTime > 0) {
        LockAssertion lock(::cs_main); // for mapBlockIndex
...
BOOST_CHECK_EQUAL(list
.

size(),

1U);
BOOST_CHECK_EQUAL(boost::get<CKeyID>(list.begin()->first)
.
...
BOOST_CHECK_EQUAL(4 * COIN, wallet->
...
->cs_wallet);
wallet->
LockCoin(COutPoint(coin.tx->GetHash(), coin.i)
);
}
}
{
...
public:
    ...
    CreateTransactionTestSetup() {
        ...
        CWallet::ScanResult result = wallet->ScanForWalletTransactions(
                ::ChainActive().Genesis()->GetBlockHash() /* start_block */, {} /* stop_block */, reserver,
                false /* update */);
        BOOST_CHECK_EQUAL(result.status, CWallet::ScanResult::SUCCESS);
    }
    ...
    bool CreateTransaction(const std::vector <std::pair<CAmount, bool>> &vecEntries, bool fCreateShouldSucceed = true,
                           ChangeTest changeTest = ChangeTest::Skip) {
        return CreateTransaction(vecEntries, {}, -1, fCreateShouldSucceed, changeTest);
    }

    bool CreateTransaction(const std::vector <std::pair<CAmount, bool>> &vecEntries, std::string strErrorExpected,
                           bool fCreateShouldSucceed = true, ChangeTest changeTest = ChangeTest::Skip) {
        return CreateTransaction(vecEntries, strErrorExpected, -1, fCreateShouldSucceed, changeTest);
    }

    bool CreateTransaction(const std::vector <std::pair<CAmount, bool>> &vecEntries, std::string strErrorExpected,
                           int nChangePosRequest = -1, bool fCreateShouldSucceed = true,
                           ChangeTest changeTest = ChangeTest::Skip) {
        CTransactionRef tx;
        CAmount nFeeRet;
        ...
        bool fCreationSucceeded = wallet->CreateTransaction(GetRecipients(vecEntries), tx, nFeeRet, nChangePos,
    ...
    std::vector <CRecipient> GetRecipients(const std::vector <std::pair<CAmount, bool>> &vecEntries) {
        ...
        for (auto entry: vecEntries) {
            vecRecipients.push_back(
                    {GetScriptForDestination(DecodeDestination(getnewaddress(JSONRPCRequest(context)).get_str())),
    ...
    std::vector <COutPoint> GetCoins(const std::vector <std::pair<CAmount, bool>> &vecEntries) {
        ...
        std::string strError;
        CCoinControl coinControl;
        BOOST_CHECK(wallet->CreateTransaction(GetRecipients(vecEntries), tx, nFeeRet, nChangePosRet, strError,
                                              coinControl));
        {
            LOCK2(wallet->cs_wallet, cs_main);
            wallet->CommitTransaction(tx, {}, {});
...
auto runTest = [&](const int nTestId, const CAmount nFeeRate,
                   const std::map<int, std::pair<bool, ChangeTest>> &mapExpected) {
...
{
0, {
true, ChangeTest::ChangeExpected}},
{
1, {
true, ChangeTest::ChangeExpected}},
{
...
{
6, {
false, ChangeTest::Skip}},
{
7, {
true, ChangeTest::NoChangeExpected}},
{
8, {
false, ChangeTest::Skip}},
{
...
auto setCoins = GetCoins({{1000,  false},
                          {5000,  false},
                          {10000, false},
                          {84000, false}});
for (
...
auto setCoins = GetCoins({{10000, false},
                          {10000, false},
                          {10000, false},
                          {10000, false},
                          {10000, false},
...
BOOST_CHECK(CreateTransaction({{10000, false},
                               {10000, false}}, false)
);
BOOST_CHECK(CreateTransaction({{10000, false},
                               {12500, true}}, true)
);
BOOST_CHECK(CreateTransaction({{10000, true},
                               {10000, true}}, true)
);
BOOST_CHECK(CreateTransaction({{1000, false},
                               {2000, false},
                               {3000, false},
                               {4000, false}}, true)
);
...
BOOST_CHECK(CreateTransaction({{1234, false},
                               {4321, false}}, true)
);
BOOST_CHECK(CreateTransaction({{1234, false},
                               {4321, false},
...
BOOST_CHECK(CreateTransaction({{25000, false},
                               {25000, false},
                               {25000, false}}, {}, 0, true, ChangeTest::ChangeExpected)
);
BOOST_CHECK(CreateTransaction({{25000, false},
                               {25000, false},
                               {25000, false}}, {}, 1, true, ChangeTest::ChangeExpected)
);
BOOST_CHECK(CreateTransaction({{25000, false},
                               {25000, false},
                               {25000, false}}, {}, 2, true, ChangeTest::ChangeExpected)
);
BOOST_CHECK(CreateTransaction({{25000, false},
                               {25000, false},
                               {25000, false}}, {}, 3, true, ChangeTest::ChangeExpected)
);
}
// Test error cases
{
...
coinControl.
nCoinType = CoinType::ONLY_NONDENOMINATED;
BOOST_CHECK(CreateTransaction({{1000, true}}, strUnableToLocateCoinJoin1, false)
);
coinControl.
nCoinType = CoinType::ONLY_FULLY_MIXED;
...
BOOST_CHECK(CreateTransaction({{5000, false},
                               {5000, false},
                               {5000, false}}, strChangeIndexOutOfRange, 4, false)
);
}
}
...
}), 2 * COIN, true /* subtract fee */}},
tx1, fee, changePos, error, dummy));
BOOST_CHECK(wallet
->CreateTransaction({
CRecipient{
GetScriptForRawPubKey({
}), 1 * COIN, true /* subtract fee */}},
...
1);
BOOST_CHECK_EQUAL(vecTallySUT
.at(0).nAmount + vecTallySUT.at(1).nAmount, (4 + 3) * COIN);
BOOST_CHECK_EQUAL(wallet
->
