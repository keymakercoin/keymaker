...
#include <config/keymaker-config.h>
#endif
...
bool RPCConsole::RPCParseCommandLine(interfaces::Node *node, std::string &strResult, const std::string &strCommand,
                                     const bool fExecute, std::string *const pstrFilteredOut,
                                     const WalletModel *wallet_model) {
    ...
    auto add_to_current_stack = [&](const std::string &strArg) {
        if (stack.back().empty() && (!nDepthInsideSensitive) &&
            historyFilter.contains(QString::fromStdString(strArg), Qt::CaseInsensitive)) {
                    ...
                    case '\n':
                        ...
                        if ((ch == ')' || ch == '\n') && stack.size() > 0) {
                            if (fExecute) {
                                ...
                                UniValue params = RPCConvertValues(stack.back()[0],
    ...
    try {
        ...
        if (executableCommand == "help-console\n") {
            Q_EMIT reply(RPCConsole::CMD_REPLY, QString(("\n"
                                                         ...
                                                         "This console can also accept RPC commands the using parenthesized syntax.\n"
                                                         "   example:    getblockhash(0)\n\n"
...
RPCConsole::RPCConsole(interfaces::Node &node, QWidget *parent, Qt::WindowFlags flags) :
    ...
    setFontSize(settings.value(fontSizeSettingsKey,
                               QFontInfo(QFontDatabase::systemFont(QFontDatabase::FixedFont)).pointSize()).toInt());
...
void
RPCConsole::setClientModel(ClientModel *model, int bestblock_height, int64_t bestblock_date, uint256 bestblock_hash,
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        setNumBlocks(bestblock_height, QDateTime::fromTime_t(bestblock_date),
                     QString::fromStdString(bestblock_hash.ToString()), verification_progress, false);
        ...
        connect(model, &ClientModel::smartnodeListChanged, this, &RPCConsole::updateSmartnodeCount);
        ...
        connect(banAction365d, &QAction::triggered, signalMapper,
                static_cast<void (QSignalMapper::*)()>(&QSignalMapper::map));
        connect(signalMapper, static_cast<void (QSignalMapper::*)(int)>(&QSignalMapper::mappedInt), this,
        ...
        connect(model->getPeerTableModel(), &PeerTableModel::layoutAboutToBeChanged, this,
...
{
    // use name for text and wallet model and internal data object (to allow to move to a wallet id later)
...
void RPCConsole::clear(bool clearHistory) {
    ...
    ui->messagesWidget->document()->setDefaultStyleSheet(
            QString(
                    "table { }"
                    "td.time { " + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_SECONDARY) +
                    " font-size: %1pt; } "
                    "td.message { " + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_PRIMARY) +
                    " font-size: %1pt; white-space:pre-wrap; } "
    ...
    message(CMD_REPLY, (tr("Welcome to the %1 RPC console.").arg(PACKAGE_NAME) + "<br>" +
                        tr("Use up and down arrows to navigate history, and %1 to clear screen.").arg(
                                "<b>" + clsKey + "</b>") + "<br>" +
...
void
RPCConsole::setNumBlocks(int count, const QDateTime &blockDate, const QString &blockHash, double nVerificationProgress,
                         bool headers) {
    if (!headers) {
...
void RPCConsole::startExecutor() {
    ...
    connect(executor, &RPCExecutor::reply, this,
            static_cast<void (RPCConsole::*)(int, const QString &)>(&RPCConsole::message));
...
void RPCConsole::updateNodeDetail(const CNodeCombinedStats *stats) {
    ...
    ui->peerLastSend->setText(stats->nodeStats.nLastSend ? GUIUtil::formatDurationStr(
            GetSystemTimeInSeconds() - stats->nodeStats.nLastSend) : tr("never"));
    ui->peerLastRecv->setText(stats->nodeStats.nLastRecv ? GUIUtil::formatDurationStr(
            GetSystemTimeInSeconds() - stats->nodeStats.nLastRecv) : tr("never"));
...
void RPCConsole::reloadThemedWidgets() {
    clear();
    ui->promptLabel->setHidden(GUIUtil::keymakerThemeActive());
...
QKeySequence RPCConsole::tabShortcut(TabTypes tab_type) const {
    switch (tab_type) {
        case TabTypes::INFO:
            return QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_I);
        case TabTypes::CONSOLE:
            return QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_C);
        case TabTypes::GRAPH:
            return QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_G);
        case TabTypes::PEERS:
            return QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_P);
        case TabTypes::REPAIR:
            return QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_R);
