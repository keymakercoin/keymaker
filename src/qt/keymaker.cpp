...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase,
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in keymaker.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in keymaker.qrc)
...
static const char *qt_argv = "keym-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info) {
    ...
    if (success) {
        ...
        // keymaker: URIs or payment requests:
        if (paymentServer) {
...
void BitcoinApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr(
            "A fatal error occurred. Keymaker Core can no longer continue safely and will quit.") + QString("\n\n") +
                                                        message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs() {
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Keymaker Core - \""),
                 ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
...
int GuiMain(int argc, char *argv[]) {
    ...
    Q_INIT_RESOURCE(keymaker);
    Q_INIT_RESOURCE(keymaker_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse keymaker.conf
    ...
    // keymaker: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
