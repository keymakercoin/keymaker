...
static const std::regex keym_names("^KEYM$|^KEYMAKER$|^wKEYM$|^WKEYM$|^KEYMcoin$|^KEYMCOIN$");


bool IsAssetNameValid(std::string name, bool isRoot) {
    if (name.length() < 3 || name.length() > 128) return false;
    if (isRoot)
        return std::regex_match(name, name_root_characters) && !std::regex_match(name, keym_names);
    else
        return std::regex_match(name, name_sub_characters) && !std::regex_match(name, keym_names); 
}
