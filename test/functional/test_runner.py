...
def main():
    ...
    # args to be passed on always start with two keymakeres; tests are the remaining unknown args
    ...
    tmpdir = "%s/keymaker_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, failfast=False, runs_ci, combined_logs_len=0):
    ...
    try:
        pidof_output = subprocess.check_output(["pidof", "keymakerd"])
