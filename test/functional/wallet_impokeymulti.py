...
"""Test the impokeymulti RPC.

Test impokeymulti by generating keys on node0, importing the scriptPubKeys and
addresses on node1 and then testing the address info for the different address
variants.

- `get_key()` and `get_multisig()` are called to generate keys on node0 and
  return the privkeys, pubkeys and all variants of scriptPubKey and address.
- `test_impokeymulti()` is called to send an impokeymulti call to node1, test
...
class ImportMultiTest(BitcoinTestFramework):
    ...
    def test_impokeymulti(self, req, success, error_code=None, error_message=None, warnings=[]):
        """Run impokeymulti and assert success"""
        result = self.nodes[1].impokeymulti([req])
        observed_warnings = []
    ...
    def run_test(self):
        ...
        self.test_impokeymulti({"scriptPubKey": {"address": address},
                               "timestamp": "now"},
                              True)
        ...
        self.test_impokeymulti({"scriptPubKey": {"address": "not valid address"},
                               "timestamp": "now"},
                              False,
                              error_code=-5,
        ...
        key = self.get_key()
        self.test_impokeymulti({"scriptPubKey": key.p2pkh_script,
                               "timestamp": "now",
                               "internal": True},
                              True)
        ...
        key = self.get_key()
        address = key.p2pkh_addr
        self.test_impokeymulti({"scriptPubKey": nonstandardScriptPubKey,
                               "timestamp": "now"},
                              False,
                              error_code=-8,
        ...
        self.test_impokeymulti({"scriptPubKey": {"address": multisig.p2sh_addr},
                               "timestamp": "now"},
                              True)
        ...
        self.test_impokeymulti({"scriptPubKey": {"address": watchonly_address},
                               "timestamp": "now"},
                              True)
        ...
        assert_raises_rpc_error(-3, 'Missing required timestamp field for key',
                                self.nodes[1].impokeymulti, [{"scriptPubKey": key.p2pkh_script}])
        assert_raises_rpc_error(-3, 'Expected number or "now" timestamp value for key. got type string',
                                self.nodes[1].impokeymulti, [{
        ...
        self.test_impokeymulti({"desc": descsum_create(desc),
                               "timestamp": "now"},
                              success=False,
                              error_code=-8,
        ...
        self.test_impokeymulti({"desc": descsum_create("pkh(" + key.pubkey + ")"),
                               "timestamp": "now",
        ...
        self.test_impokeymulti({"timestamp": "now"},
                              success=False,
                              error_code=-8,
        ...
        self.test_impokeymulti({"desc": descsum_create("multi(1," + key1.pubkey + "," + key2.pubkey + ")"),
                               "timestamp": "now"},
                              True,
                              warnings=["Some private keys are missing, outputs will be considered watchonly. If this is intentional, specify the watchonly flag."])
        ...
        result = self.nodes[0].impokeymulti(
            [{
