# Support for Output Descriptors in Keymaker Core

Since Keymaker Core v0.17 with Future Lock Transactions, 
...
existing Keymaker Core wallets, a convenience function `combo` is
...
All RPCs in Keymaker Core will include the checksum in their output. Only
certain RPCs require checksums on input, including `deriveaddress` and
`impokeymulti`. The checksum for a descriptor without one can be computed
